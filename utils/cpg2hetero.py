...
class CPGHeteroDataset(InMemoryDataset):
    def __init__(self, root='./CPG', transform=None, pre_transform=None, 
        ...
        self.test_n = int(test_n) if test_n is not None else 0
    ...
    def process(self):
        ...
        def line_generator(path, max_lines=None):
        ...
        max_lines = self.test_n if getattr(self, 'test_n', 0) > 0 else None

        if active_workers > 1:
            with Pool(processes=active_workers) as pool:
                results_iter = pool.imap_unordered(_process_single_record, line_generator(raw_path, max_lines=max_lines), chunksize=128)

                for result, status in tqdm(results_iter, total=total_lines, desc="Processing"):
                    if status == 'success' and result is not None:
                        idx, raw_graph, metadata = result
                        
                            # 1. Convert to Tensor (Main Process) but do NOT encode now (batch later)
            ...
            for line_data in tqdm(line_generator(raw_path, max_lines=max_lines), total=total_lines):
        ...
        # Collate and Save
        data_list_path = os.path.join(self.processed_dir, "data_list.pt")
        print(f"Saving data_list to {data_list_path} before collate...")
        ...
        # Cleanup saved data_list after successful processing
