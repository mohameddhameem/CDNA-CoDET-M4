...
# ============================================================
# This script processes CPG data into language-specific homogeneous graphs
#
# Usage:
#   bash scripts/generate_homo.sh                          # Processes python language
#   CODE_LANG=java WORKERS=64 bash scripts/generate_homo.sh
#   CODE_LANG=cpp SAVE=CPG bash scripts/generate_homo.sh
#
...

{
    ...
    echo "Output: ${SAVE}/processed_homo_${CODE_LANG}/"
    if [ "${RESUME_MODE}" = "enabled" ] && [ -f "${SAVE}/processed_homo_${CODE_LANG}/processed.pt" ]; then
