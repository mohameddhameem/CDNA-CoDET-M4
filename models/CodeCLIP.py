...
class Downstream(nn.Module):
    def __init__(self, pretrained_model, args):
        ...
        # Router takes normalized concatenated features to avoid scale dominance
        self.router = nn.Sequential(
            nn.Linear(embed_dim * 2, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, 2),
        )
        # Zero bias + small weight init keeps early routing near 0.5/0.5
        nn.init.zeros_(self.router[-1].bias)
        nn.init.normal_(self.router[-1].weight, std=0.01)

        self.expert_graph = nn.Sequential(nn.Linear(embed_dim, hidden_dim), nn.GELU())
        self.expert_text  = nn.Sequential(nn.Linear(embed_dim, hidden_dim), nn.GELU())
        self.classifier   = nn.Linear(hidden_dim, num_classes)

        # Regularization weights — tune via args if needed
    ...
    def forward(self, batch, return_aux=False):
        with torch.no_grad():
            ...
            feat_t = self.text_encoder(batch.code)

        # Normalize before concat so neither modality dominates the router by scale
        router_input = torch.cat([F.normalize(feat_g, dim=-1),
                                   F.normalize(feat_t, dim=-1)], dim=-1)

        raw_weights = F.softmax(self.router(router_input), dim=-1)

        # Clamp to [min_weight, 1-min_weight] so neither expert is fully ignored
        routing_weights = self.min_weight + (1 - 2 * self.min_weight) * raw_weights

        print(f"\nRouting weights: graph={routing_weights.mean(dim=0)[0]:.4f}, text={routing_weights.mean(dim=0)[1]:.4f}")

        fused = (routing_weights[:, 0:1] * self.expert_graph(feat_g)
               + routing_weights[:, 1:2] * self.expert_text(feat_t))
