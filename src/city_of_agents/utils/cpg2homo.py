...
def _process_single_record(line_data):
    ...
    try:
        ...
        # Pack metadata (Defense against None using 'or')
        metadata = {
        ...
        # Note: We cannot apply pre_transform here easily because it usually expects a Data object
        # We will apply it in the main process.
            ...
            with Pool(processes=active_workers) as pool:
                ...
                for result, status in tqdm(results_iter, total=total_lines, desc="Processing"):
                    if status == 'success' and result is not None:
                        ...
                        # --- CRITICAL: Convert to Tensor in MAIN Process ---
                        # Create edge_index tensor (2 x E)
